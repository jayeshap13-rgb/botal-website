...
// EDIT: Change this admin password before sharing the website preview.
const ADMIN_PASSWORD = "botal123";
...
function CartPanel({ cart, onQuantity, onRemove, onCheckout }) {
  ...
  return (
    <aside className="glass-panel reveal rounded-[1.5rem] p-5">
      <div className="flex items-start justify-between gap-4">
        <div>
          <p className="text-xs font-black uppercase tracking-[0.14em] text-deepOlive">Shopping Cart</p>
          <h3 className="mt-2 text-2xl font-black text-navyInk">Checkout products</h3>
        </div>
      ...
      <div className="mt-5 grid max-h-80 gap-3 overflow-auto pr-1">
        {cart.length ? (
          ...
          <p className="rounded-2xl bg-white p-5 font-bold text-ink/60">Your cart is ready for handmade glass goodness.</p>
        )}
      </div>
...
function Products({ products, categories, cart, onAddToCart, onQuantity, onRemoveFromCart, onCheckout }) {
    ...
    <section id="products" className="bg-cream py-20 sm:py-28">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-10 grid gap-8 xl:grid-cols-[1fr_24rem] xl:items-start">
          ...
          <CartPanel cart={cart} onQuantity={onQuantity} onRemove={onRemoveFromCart} onCheckout={onCheckout} />
        </div>
      </div>
...
function AdminPanel({ products, categories, orders, onSaveProduct, onResetProducts, onDeleteProduct, onClearOrders, onUpdateOrderStatus, onAddCategory, onDeleteCategory, onResetCategories }) {
